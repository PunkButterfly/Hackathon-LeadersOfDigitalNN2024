...
class DataProcessorV2:
    ...
    def process(self, transactions: pd.DataFrame, clients: pd.DataFrame, mode: str = 'inference'):
        ...
        if mode == 'train':
            ...
            clients_on_pension_df['accnt_end_date'] = clients_on_pension_df['accnt_bgn_date'] + pd.to_timedelta(clients_on_pension_df['cprtn_prd_d'], unit = 'days')

            clients_not_on_pension_df = clients[clients['accnt_status'] == 'Накопительный период'].copy()
            clients_not_on_pension_df['accnt_end_date'] = clients_not_on_pension_df['accnt_bgn_date'] + (clients_not_on_pension_df['pnsn_age'] - clients_not_on_pension_df['age_agreement_start']).apply(lambda x: pd.to_timedelta(int(x)*365, unit = 'days'))

            #pd.to_datetime('2030-01-01') #заглушка
            clients = clients.merge(pd.concat([clients_on_pension_df, clients_not_on_pension_df]).loc[:, ['clnt_id', 'accnt_end_date']], on='clnt_id', how='left')
            ...
            clients = clients[clients['age_over_accnt_end_flg'] == 1]
...
df = DataProcessorV2('backend/models/configs/', 'processor_v2.yaml').process(transactions=None, clients=pd.read_csv('sample_cntrbtrs_clnts_ops_trn-3.csv', sep = ';', encoding = 'cp1251'), mode='train')
print(df)